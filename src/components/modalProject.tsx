    ...
    return (
        isShowModalProject ? (
            <AnimatePresence>
                ...
                >
                    <section 
                        className="w-10/12 md:w-9/12 lg:w-7/12 h-[400px] md:h-[480px] mx-auto my-20 overflow-y-auto shadow-md bg-gray-100 rounded-lg md:pt-10 flex flex-col justify-between"
                     ...
                     >
                        <header className="w-12/12 md:w-max mx-auto">
                            ...
                            <h1 className="text-2xl font-semibold text-center mt-2 text-black dark:text-gray-800">{project.title}</h1>
                        </header>
                        <section className="text-center my-5 px-3 md:px-12">
                            <p className="text-black dark:text-gray-800">{project?.desc}</p>
                        </section>
                        <footer className="flex items-center bg-white w-full h-10 text-center">
                            <Link href={project.link.github}>
                                <a className="font-semibold h-full border-r-2 w-4/12 py-2 hover:bg-gray-300 text-black no-underline text-sm">Github</a>
                            </Link>
                            <Link href={project.link.demo}>
                                <a className="font-semibold h-full border-r-2 w-4/12 py-2 hover:bg-gray-300 text-black no-underline text-sm">Demo</a>
                            </Link>
                             <a onClick={handleCloseModalProject} className="font-semibold h-full w-4/12 py-2 hover:bg-gray-300 text-black no-underline text-sm">Yes, I Got it</a>
                        </footer>
                    </section>
                </motion.section>
            </AnimatePresence>
