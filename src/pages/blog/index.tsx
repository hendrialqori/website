...
const InputSearch = memo(function _({
  value,
  handleChange,
...
export default function Blog({ articles }: ArticlesProps) {
      ...
      <>
        ...
        <header className="mt-5">
          ...
          <p className="text-black dark:text-light/70">
            I write blog, Share a bit of my knowledge about web development
            relative topics, Enjoy it fellas!
          </p>
        </header>
            ...
            Articles.map((article, i) => (
              <Link key={i} href={"/blog/" + article.slug}>
                <article aria-label="card-wrapper" role={"button"} tabIndex={0}>
                  ...
                  <h2 className="font-extrabold text-xl hover:underline">
                    {article.title}
                  </h2>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const articles = files.map((file: string) => {
    const dataWithMatter = fs.readFileSync(
      path.join("src", "articles", file),
      "utf-8"
    );
