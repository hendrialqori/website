    ...
    <Layout title='About'>
      <section>
        <section className='mb-10' aria-label='top-side'>
          ...
          <p className='text-[17px] my-5 text-gray-800 dark:text-gray-100'>
          ...
          <p className='text-[17px] text-gray-800 dark:text-gray-100'>
            I love learning something new about programming topic, Whatever!. <br />
          <span className='bg-yellow-300 px-1 rounded-sm dark:text-black'>Javascript</span>, <span className='bg-sky-600 px-1 text-white rounded-sm'>Typescript</span>, <span className='bg-sky-400 px-1 rounded-sm text-white'>React</span> and <span className='bg-black text-white px-1 rounded-md'>next</span>, i love all those. <br />
            And now i <span className='bg-green-300 px-1 font-semibold rounded-sm text-black'>open to work as Frontend developer</span> with above skill.
          </p>
        </section>
        <section aria-label='bottom-side'>
          <h1 className='my-5 text-sky-400 dark:text-sky-400 font-semibold text-xl'>#betterTogether</h1>
          <p className='text-[17px] text-gray-800 dark:text-gray-100'>If you want to make friendship or cooperation, please contact me on my social media accounts below.</p>
          <section className='flex gap-5 mt-3' aria-label='contact-wrapper'>
            <div className='grid gap-1'>
              {Contacts.slice(0, 3).map((contact, i) => (
                <a className='text-[17px] font-semibold text-sky-600 dark:text-sky-400' href={contact.link} target='_blank' key={i} rel="noreferrer">
                  {contact.title}
                </a>
              ))}
            </div>
